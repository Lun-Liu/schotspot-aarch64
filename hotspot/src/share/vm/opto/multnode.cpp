...
//-------------------------------is_sc_check_if_pattern-------------------------
// Return true  for "sccheck -> Bool ->if(test)-> proj -> 
//                                       |
//                                       V
//                                     other_proj->[region]-> call_static_java"
//
// "must_reason_predicate" means the uct reason must be Reason_predicate
bool ProjNode::is_sc_check_proj() {
#ifndef PRODUCT
      tty->print("check proj: ");
      this->dump();
      tty->print_cr("");
#endif
  Node *in0 = in(0);
  ...
  Node* in00 = in0 -> in(0);
  if(in00 == NULL || !in00->is_Bool()) return false;
  Node* in000 = in00->in(0);
  if(in000 == NULL || !in000 -> is_SCCheck()) return false;
  ...
  for (int ct = 0; ct < path_limit; ct++) {
    ...
    if (out->is_CallStaticJava()) {
#ifndef PRODUCT
      tty->print_cr("found sccheck proj");
      this->dump_spec(tty);
#endif
      return true;
    }
